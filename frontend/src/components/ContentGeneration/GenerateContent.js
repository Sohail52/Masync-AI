...
const BlogPostAIAssistant = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const prompt = `write a long blogpost about ${values.prompt}`;
      const generatedContent = await mutation.mutateAsync({ prompt });
      console.log('Generated Content:', generatedContent); // Log the response
        ...
        <div className='flex mt-3'>
          ...
          <div className='mr-2 mb-2'>
            <span className='text-sm font-semibold bg-green-200 px-4 py-2 rounded-full'>
              ...
              {data?.user?.monthlyRequuestCount}
            </span>
          </div>
        </div>
